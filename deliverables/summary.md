...
donors) for **brakes** on human CD4⁺ T-cell function — the target class
checkpoint-blockade immunotherapy drugs. We build a **signed causal map** and
prioritize a shortlist from it.

The method is the point. At 2.6 million cells, 97.5% of tested knockdowns clear
q < 0.05 — so we rank by **causal effect size** (power-equalized
...
backed by versioned, reproducible code.
