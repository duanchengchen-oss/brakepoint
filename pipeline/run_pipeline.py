...
def qc_normalize(adata, control):
    import scanpy as sc
...
def effect_sizes(adata, control, pert_key="perturbation", donor_key="donor", min_cells=30):
...
def knockdown_check(adata, df, control, modality="KO", pert_key="perturbation"):
    ...
    counts = np.asarray(counts.todense()) if hasattr(counts, "todense") else np.asarray(counts)
    cp = counts / (counts.sum(1, keepdims=True) + 1e-9)     # per-cell fraction
    labels = adata.obs[pert_key].astype(str).values
    ...
    for p in df["perturbation"]:
        ...
        if gene in adata.var_names:
            ...
            m_p = cp[labels == p, gi].mean(); m_c = cp[ctrl_mask, gi].mean()
...
def main():
    ...
    adata = qc_normalize(adata, a.control)
    df = effect_sizes(adata, a.control)
